...
class CompanyFormController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('training_and_placement_cell.form.companyForm', compact('branches'));
    }
    ...
    public function store(Request3 $request, Request $request1) {
        ...
        $counter12 = $request::input('dummy12');    
        for($b = 1; $b <= $counter12; $b++){
            if($request::input('round'.$b)) {
                CompanyRounds::create([
                    'company_id' => $request::input('comID'),
                    'round_name' => $request::input('round'.$b),
                    'round_id' => $request::input('comID').$b
                    ]);
            }
