...
$(document).on('click', '#removeRound', function() {
  var $round = $(this).parents('.rounds');
  $round.remove();
});

function cloneRound() {
  counter12 += 1;
  $('#roundClone').append($('.rounds:eq(0)').clone(true));
  $(".rounds:last").show();
  $(".rounds:last").find('[id]').each(function () {
    var id = $(this).attr('id');
    var id1 = $(this).attr('id') + counter12;
    $(this).attr('id', id).attr('name', id1);
  });
  // console.log(counter);
  $('#roundClone').find('#dummy12').attr('value', counter12);
}  
//---------------------
